...
%>
                    ...
                    <div class="portlet">
                        ...
                        <div class="portlet-body">
                            <div class="">
                                ...
                                <button style="margin-left: 50px" class="btn blue" id="findlikeBtn">查询</button>
                                <button  class="btn blue" id="showAllBtn">显示全部</button>
                                <button  class="btn blue" id="addSupplierBtn" data-toggle="modal" data-target="#myModa1" >添加匹配店铺</button>
                            </div>
                         </div>
                    </div>
                ...
                <div class="span12" style="padding-left: 2%;padding-right:2%">
                    ...
                    <div class="portlet paddingless">
                        <div class="portlet-title line">
                            <div class="caption"><i class="icon-bell"></i>选择商品</div>      
                        </div>
                            <div class="portlet-body">
                                    ...
                                    <div class="modal-dialog">
                                        <div class="modal-content">
                                            ...
                                            <div class="modal-body">
                                                ...
                                                <button type="button" class="btn btn-primary" id="backBtn" data-dismiss="modal">返回</button>
                               					</div>
    ...
    <div class="subdiv">
    	<h3 style="float: left;margin-left: 50px">共<span id="goodsNum"></span>件商品，购买数量为：<span id="totalNum"></span>件，合计:￥<span id="totalPrice"></span>元</h3>
    	<input type="button" class="btn red" value="提交订单" style="float: left;margin-left: 320px;margin-top: 15px" id="subScheme" />
    	<input type="button" class="btn red" value="平台优选方案A" style="float: left;margin-left: 20px;margin-top: 15px" id="schemeA" />
    	<input type="button" class="btn red" value="平台优选方案B" style="float: left;margin-left: 20px;margin-top: 15px" id="schemeB" />
    	<input type="button" class="btn blue" value="重新计算" style="float: left;margin-left: 20px;margin-top: 15px" id="calculateNum" />
    </div>
                                   ...
                                   function getData() {	
                                    ...
                                    function getTable() {
    						            ...
    						            for(var j=0;j<buyPlanList.length;j++){
    						                ...
    						                html += "<tr index='"+buyPlanList[j].comName+"'><td>"+buyPlanList[j].comName+"</td><td>"+buyPlanList[j].spec+"</td><td>"+buyPlanList[j].evaluate+"<p>——<p><span class='priceDifference'></span></td><td>"+buyPlanList[j].buyNum+"</td>";
    						                		...
    						                		if(data[j][m]==null){
    						                			...
    						                			if(data[j][m].supp.name==name){
    						                				if(data[j][m].stock>=buyPlanList[j].buyNum){
    						                					html += "<td class='selectData selected' index='"+buyPlanList[j].id+"' supplierId='"+data[j][m].id+"' evaluate='"+buyPlanList[j].evaluate+"'>单价<span>"+data[j][m].price+"</span>库存"+data[j][m].stock+"<br/>"+data[j][m].expiryDate+"<p>购买数量<input type='text' class='inp' value='"+buyPlanList[j].buyNum+"'></td>"
    						                				}else{
    						                					html += "<td class='selectData selected' index='"+buyPlanList[j].id+"' supplierId='"+data[j][m].id+"' evaluate='"+buyPlanList[j].evaluate+"'>单价<span>"+data[j][m].price+"</span>库存"+data[j][m].stock+"<br/>"+data[j][m].expiryDate+"<p>购买数量<input type='text' class='inp' value='"+data[j][m].stock+"'></td>"
    						                				}
    						                			}else{
    						                				html += "<td class='selectData' index='"+buyPlanList[j].id+"' supplierId='"+data[j][m].id+"' evaluate='"+buyPlanList[j].evaluate+"'>单价<span>"+data[j][m].price+"</span>库存"+data[j][m].stock+"<br/>"+data[j][m].expiryDate+"<p>购买数量<input type='text' class='inp'></td>"
    						                			}
    						                		}
    						                }
    						                html += "</tr>"
						            ...
						            function getTotalPrice() {
						            	...
						            	$('td.selected').each(function(){
						            		...
						            		/*计算选中每种商品与估价差异*/
						            		var differenceValue = $(this).find("span").html()-$(this).attr("evaluate");
						            		if(differenceValue>0){
						            			$(this).parent().find(".priceDifference").html("+"+Math.round(differenceValue*100)/100);
						            			$(this).parent().find(".priceDifference").addClass('positiveNumber');
						            		}else{
						            			$(this).parent().find(".priceDifference").html(Math.round(differenceValue*100)/100);
						            			$(this).parent().find(".priceDifference").addClass('negative');
						            		}
						            	})
